...
# This data library is taken from https://github.com/magenta/magenta/blob/main/magenta/models/music_vae/data.py to process our tfrecord data
# Remove unused functions in this implementation such as TrioConverter, etc
import abc
import collections
import copy
import functools
...
def get_dataset(config,tf_file_reader=tf.data.TFRecordDataset,is_training=False,cache_dataset=True):
...
def _get_input_tensors(dataset, config):
  ...
  return {
      'input_sequence': input_sequence, # <tf.Tensor: shape=(512, 64, 27), 64->sequence length
      'output_sequence': output_sequence, # <tf.Tensor: shape=(512, 64, 27)
      'control_sequence': control_sequence, # None
      'sequence_length': sequence_length # <tf.Tensor: shape=(512,),
  }

def dataset_tensor(tfrecords_path,batch_size):
    
    config = {
        'batch_size': batch_size,
        'note_sequence_augmenter': None,
        'train_examples_path': tfrecords_path,
        'data_converter' : GrooveConverter(split_bars=4, steps_per_quarter=4, quarters_per_bar=4,
        max_tensors_per_notesequence=20,
        pitch_classes=ROLAND_DRUM_PITCH_CLASSES,
        inference_pitch_classes=REDUCED_DRUM_PITCH_CLASSES)

    }
    data = get_dataset(config,is_training=True)
    dataset = _get_input_tensors(data,config)
    return dataset
